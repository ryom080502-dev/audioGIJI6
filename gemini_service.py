...
class GeminiService:
    def __init__(self):
        ...
        # プロンプトテンプレート
        self.analysis_prompt = """あなたは優秀な書記です。以下の音声内容を全て解析し、わかりやすく整理された議事録を作成してください。

【重要な指示】
1. 音声の内容を全て聞き取り、漏れなく記録してください
2. 箇条書きには「・」（中黒）のみを使用してください
3. 「*」（アスタリスク）や「#」（ハッシュ）は絶対に使用しないでください
4. 5つのセクションには「## 1.」～「## 5.」の形式のみを使用してください
5. 各セクションは明確に分けて、読みやすく構成してください
6. 決定事項とネクストアクションは特に明確に記載してください

【出力形式】
## 1. 会議の概要
（会議の目的、参加者、主なテーマを2-3行で簡潔に記載）
...
音声内容を解析してください。"""
        ...
        self.merge_prompt = """以下は同じ会議を時間ごとに分割して解析した複数の議事録です。
これらを統合して、読みやすく整理された1つの議事録にまとめてください。

【統合の指示】
1. 重複する内容は削除し、一度だけ記載する
2. 時系列順に整理し、会議の流れが分かるようにする
3. 箇条書きには「・」（中黒）のみを使用する
4. 「*」（アスタリスク）や「#」（ハッシュ）は絶対に使用しない
5. 5つのセクションには「## 1.」～「## 5.」の形式のみを使用する
6. 決定事項とネクストアクションは特に丁寧にまとめる
        ...
        try:
            ...
            try:
                response = self.model.generate_content(
                    [self.analysis_prompt, audio_file],
